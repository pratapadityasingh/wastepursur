...
const Product = () => {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                const response = await fetch(`http://localhost:5000/api/products/getbyidproduct/${id}`);
    ...
    const addToCart = async () => {
        ...
        try {
            await axios.post(`http://localhost:5000/api/cart/addtocart/${decodedPayload.userId}/${product._id}`);
            setIsSuccess(true);
            router.push("/addcart");
        } catch (error) {
